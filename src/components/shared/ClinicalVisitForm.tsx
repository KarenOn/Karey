...
export default function ClinicalVisitForm({ petId, visitId, vets, initialValues, currentUserRole, currentUserId, onSaved, onCancel }: ClinicalVisitFormProps) {
  ...
  const lockedVetId =
    !visitId && currentUserRole === "vet" ? (currentUserId ?? "") : null;
  const vetFieldDisabled = lockedVetId !== null;
  useEffect(() => {
    setValues({
      ...emptyValues,
      ...initialValues,
      ...(lockedVetId ? { vetId: lockedVetId } : {}),
    });
  }, [initialValues, lockedVetId, visitId]);
  const change = (event: FormFieldChangeEvent) =>
    setValues((current) => ({
      ...current,
      [event.target.name]: event.target.value,
    }));
  const selectAttachment = (file: LocalFile | null) => {
  ...
  async function submit(event: React.FormEvent) {
    ...
    const parsed = ClinicalVisitCreateSchema.safeParse({
      ...values,
      vetId: lockedVetId ?? values.vetId,
      visitAt: values.visitAt ? new Date(values.visitAt) : undefined,
      weightKg: values.weightKg === "" ? undefined : Number(values.weightKg),
      temperatureC:
        values.temperatureC === "" ? undefined : Number(values.temperatureC),
    });
    if (!parsed.success) {
      setError(
        parsed.error.issues[0]?.message ?? "Revisa los datos de la visita.",
      );
      return;
    }
    setSaving(true);
    ...
    try {
      const uploaded = attachment
        ? await uploadFileToStorage(
            attachment,
            UPLOAD_SCOPES.medicalAttachment,
            visitId,
          )
        : null;
      const response = await fetch(
        visitId ? `/api/visits/${visitId}` : `/api/pets/${petId}/visits`,
        {
      ...
      if (!response.ok && uploaded)
        await fetch("/api/uploads/object", {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ storageRef: uploaded.storageRef }),
        }).catch(() => undefined);
      const data = (await response.json().catch(() => null)) as {
        message?: string;
      } | null;
      if (!response.ok)
        throw new Error(data?.message ?? "No se pudo guardar la visita.");
    ...
    <form onSubmit={submit} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
      ...
      <FormField
        ...
        options={vets.map((vet) => ({
          value: vet.id,
          label: vet.name,
          keywords: vet.email ? [vet.email] : [],
        }))}
        onChange={change}
        disabled={vetFieldDisabled}
        placeholder="Selecciona el veterinario"
        required
      />
      {vetFieldDisabled ? (
        <p className="-mt-2 text-xs text-muted-foreground">
          Asignado a tu usuario veterinario.
        </p>
      ) : null}
      <FormField
      ...
      {/* <div className="space-y-2 sm:col-span-2">
        <p className="text-sm font-semibold">Documento adjunto</p>
        <SignedFileUploader
          accept="image/*,.pdf,.doc,.docx,.xls,.xlsx,.txt"
          onError={setError}
          onFileSelected={selectAttachment}
        />
        {attachment ? (
          <p className="text-sm text-muted-foreground">
            {attachment.fileName} · {Math.ceil(attachment.file.size / 1024)} KB
          </p>
        ) : null}
      </div> */}
      <div className="space-y-2 sm:col-span-2">
