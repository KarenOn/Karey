...
import { apiSearchClients } from "@/lib/api/clients";
...
export default function ClinicalReportDialog({ open, onClose, pets, clients, initialPetIds = [] }: Props) {
  ...
  const clientOptions = useMemo(() => {
    ...
    if (selectedClient && !options.some((option) => option.value === String(selectedClient.id))) {
      options.unshift({ value: String(selectedClient.id), label: selectedClient.fullName, keywords: [selectedClient.phone ?? "", selectedClient.email ?? ""] });
    }
    return options;
  }, [searchedClients, selectedClient]);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      void apiSearchClients(clientSearch, 20)
        .then(setSearchedClients)
        .catch(() => setSearchedClients([]))
        .finally(() => setClientLoading(false));
    }, 300);
    return () => window.clearTimeout(timer);
  }, [open, mode, clientSearch]);

  function reset() { setSelectedIds(initialPetIds); setClientId(""); setSelectedClient(null); setClientSearch(""); setSearchedClients([]); setMode("patients"); setRangeMode("today"); setDate(new Date().toISOString().slice(0, 10)); setFrom(""); setTo(""); }
  ...
  return <Modal open={open} onClose={close} title="Generar informe clínico" description="Selecciona el alcance y las fechas del historial que quieres revisar o descargar." size="lg" footer={<div className="flex gap-3"><Button variant="outline" onClick={close}>Cancelar</Button><Button onClick={submit} disabled={busy || (mode === "patients" ? !selectedIds.length : !clientId)}>{busy ? <LoaderCircle className="mr-2 h-4 w-4 animate-spin" /> : <FileText className="mr-2 h-4 w-4" />}Generar informe</Button></div>}>
    <div className="space-y-5">
      <fieldset className="space-y-3"><legend className="text-sm font-semibold">Modo</legend><label className="flex cursor-pointer items-start gap-3 rounded-lg border border-border p-3"><input type="radio" checked={mode === "patients"} onChange={() => setMode("patients")} className="mt-1" /> <span><span className="block text-sm font-medium">Pacientes seleccionados</span><span className="text-xs text-muted-foreground">{selectedPets.length ? selectedPets.map((pet) => pet.name).join(", ") : "Selecciona uno o varios en la tabla."}</span></span></label><label className="flex cursor-pointer items-start gap-3 rounded-lg border border-border p-3"><input type="radio" checked={mode === "client"} onChange={() => setMode("client")} className="mt-1" /> <span className="w-full"><span className="block text-sm font-medium">Todos los pacientes de un cliente</span><span className="mt-2 block"><SearchableSelect value={clientId} onValueChange={(value, detail) => { setClientId(value); setSelectedClient(detail?.option ? { id: Number(detail.option.value), fullName: detail.option.label, phone: detail.option.keywords?.[0] || null, email: detail.option.keywords?.[1] || null } : null); }} onSearchChange={setClientSearch} options={clientOptions} loading={clientLoading} emptyMessage="Sin clientes encontrados." placeholder="Seleccionar cliente" searchPlaceholder="Buscar por nombre, teléfono o correo..." /></span></span></label></fieldset>
