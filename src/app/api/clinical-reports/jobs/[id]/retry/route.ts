...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const updated = await prisma.clinicalReportJob.updateMany({ where: { id, clinicId, status: ClinicalReportJobStatus.FAILED }, data: { status: ClinicalReportJobStatus.PENDING, error: null, storageRef: null, completedAt: null } });
