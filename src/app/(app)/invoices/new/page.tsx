...
import NewInvoicePOSPage from "./NewInvoiceClient";
...
//   useEffect(() => {
//     if (!prefilledClientId || loading || clients.length === 0) return;

//     const nextClientId = Number(prefilledClientId);
//     if (!Number.isFinite(nextClientId)) return;
//     if (!clients.some((client) => client.id === nextClientId)) return;

//     setClientId(String(nextClientId));
//   }, [clients, loading, prefilledClientId]);
...
//   useEffect(() => {
//     if (!prefilledPetId || pets.length === 0) return;

//     const nextPetId = Number(prefilledPetId);
//     if (!Number.isFinite(nextPetId)) return;
//     if (!pets.some((pet) => pet.id === nextPetId)) return;
...
//         {prefilledAppointmentId || prefilledTodayTurnId ? (
//           <div className="rounded-2xl border border-sky-200 bg-sky-50/80 p-4 text-sm text-sky-900">
//             <p className="font-semibold">Facturación iniciada desde Hoy</p>
